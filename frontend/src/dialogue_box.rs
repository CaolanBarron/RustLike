use std::{io::stdout, iter::Enumerate};
...
impl DialogueBox {
    pub fn draw_title(&self, title: &String) {
        ...
        for i in 0..((self.end_position.x - 2) - (self.start_position.x)) {
            execute!(
                stdout(),
                MoveTo((self.start_position.x + 1) as u16 + i as u16, start_y + 2),
                Print('\u{2504}')
            );
        }
    }
    ...
    pub fn draw_options(&self, lines: &Vec<String>, choice: &u16) {
        ...
        for (i, line) in lines.iter().enumerate() {
            execute!(
                stdout(),
                MoveTo(start_x, start_y + (2 * i as u16)),
                Print(line)
            );
        }
    ...
    pub fn clear_box(&self) {
        for i in self.start_position.y + 1..self.end_position.y - 1 {
            execute!(
                stdout(),
                MoveTo((self.start_position.x + 2) as u16, (i).try_into().unwrap()),
                Print("            ".to_string())
            );
        }
    }
    ...
    pub fn draw_pointer(&self, choice: &u16) {
        execute!(
            stdout(),
            MoveTo(
                self.start_position.x as u16 + 3,
                (self.start_position.y as u16 + 6) + (2 * choice)
            ),
            Print('\u{25B6}')
        );
    }
...
mod dialogue_box_tests {
    ...
    use crossterm::terminal::{enable_raw_mode, Clear, EnterAlternateScreen};
